...
namespace GamingGarrison
{
    ...
    {
        internal static bool s_needsStaggerOddToEvenConversion;
        internal static StaggerAxis s_staggerAxis;
        internal static bool s_needsGridRotationToMatchUnityIsometric;
        internal static bool s_needsGridRotationToMatchUnityStaggeredIsometric;
        ...
        internal static bool ImportTileLayer(TMX.Layer layer, GameObject parent, int layerID, bool infinite, out GameObject newLayer)
        {
            ...
            if (s_staggerAxis != StaggerAxis.None)
            {
                HandleStaggerOffsetting(tileGrid.cellSize, out gridXOffset, out gridYOffset, out offsetPosition);
            }
        ...
        static void HandleStaggerOffsetting(Vector3 cellSize, out int gridXOffset, out int gridYOffset, out Vector3 offsetPosition)
        {
            if (s_needsStaggerOddToEvenConversion) // When odd
            {
                if (s_staggerAxis == StaggerAxis.X)
                {
                    gridXOffset = 1;
                    gridYOffset = 0;
        ...
        static bool FillTilemapFromData(Tilemap tilemap, int startX, int startY, int width, int height, uint[] data)
        {
            ...
            for (int i = 0; i < data.Length; i++)
            {
                ...
                if (importedTile != null && importedTile.tile != null)
                {
                    ...
                    else if (s_needsGridRotationToMatchUnityStaggeredIsometric)
                    {
                        pos = new Vector3Int(startX + (i % width), startY + (i / width), 0);
                        Vector3Int newPos = ConvertIsometricStaggerToUnityIso(pos);
                        pos = newPos;
                    }
                    if (s_staggerAxis == StaggerAxis.X)
                    {
            ...
            if (anyTilesWithCollision)
            {
                ...
                {
                    if (TiledTMXImporter.s_useCompositeColliders)
                    {
        ...
        static Vector3Int ConvertIsometricStaggerToUnityIso(Vector3Int gridPos)
        {
            // Tiled can handle isometric in a staggered space, whereas Unity isometric is always a rotated grid...
            // We need to convert staggered coordinates into their Unity grid equivalents
            int staggerIndex = s_needsStaggerOddToEvenConversion ? 0 : 1;
            if (s_staggerAxis == StaggerAxis.Y)
            {
                Vector3Int pos = new Vector3Int(gridPos.x - (gridPos.y / 2), -(gridPos.y / 2) - gridPos.x, gridPos.z);
                if ((gridPos.y % 2) == staggerIndex)
                {
                    pos.x += 1;
                }
                return pos;
